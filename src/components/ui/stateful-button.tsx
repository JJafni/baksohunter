...
export function StatefulButton({
  ...
  const { onClick: _onClick, onDrag, onDragStart, onDragEnd, onAnimationStart, onAnimationEnd, ...buttonProps } = props
  ...
  return (
    <motion.button
      ...
      className={cn(
        ...
        isLoading && 'cursor-wait border-[#b8954a] shadow-[0_0_28px_rgba(184,149,74,0.3)]',
        className,
      )}
      {...buttonProps}
      onClick={handleClick}
...
function ButtonIcon({ icon }: { icon: 'sword' | 'shield' }) {
  const className =
